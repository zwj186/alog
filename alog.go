...
func NewALog(configs ...string) *ALog {
	...
	cfgFile := new(log.ConfigFile)
	config := log.LoadDefaultConfig()
	if len(configs) > 0 {
		cfgFile.FilePath = configs[0]
		fileInfo, serr := os.Stat(configs[0])
		if serr == nil {
			cfgFile.ModTime = fileInfo.ModTime().Unix()
		}
		err := utils.NewConfig(cfgFile.FilePath).Read(config)
		if err != nil {
			panic(err)
		}
	...
	return &ALog{
		config: config,
		manage: manage.NewLogManage(config, cfgFile),
		tag:    log.DefaultTag,
		cfile:  cfgFile,
	}
}
...
func (a *ALog) ReloadConfig(cfg string) error {
	...
	a.config = config
	a.manage.Stoped()
	a.manage = manage.NewLogManage(config, a.cfile)
	return nil
}
